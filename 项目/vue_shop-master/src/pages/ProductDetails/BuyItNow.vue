<template>
  <div>
    ...
    <div class="container mx-auto px-36 py-10">
      <p class="text-base font-semibold pb-2 border-b-2 flex justify-between"><span>确认收货地址</span> <span @click="toAddr" class=" text-blue-500 cursor-pointer hover:text-indigo-700">管理收货地址</span> </p>
      <div class="flex flex-col mb-16">
        <el-radio-group v-model="radio">
          <p class="my-6" v-for="item in addr" :key="item.addr_id">
            <el-radio :label="item.addr_id">
              <span class="text-2xl font-extrabold">
                {{ item.addr_local }}({{ item.addr_person }} 收){{
                  item.addr_phone
                }}</span
              >
            </el-radio>
        ...
        >
          <div class="w-full flex flex-col justify-center items-center">
            <div class="w-2/3 mb-4 flex">
              <div class="w-2/6 pr-5 flex justify-end">
                <span class=""
                  ><span class="text-red-600">*</span> 地址信息:</span
                >
              </div>
              <div class="w-4/6">
                <select
                  ...
                  ref="sheng"
                  class="w-1/3 border p-2 text-black"
                >
                ...
                <select
                  ...
                  class="w-1/3 border p-2 text-black"
                >
                  <option value="" ref="shi1">---请选择---</option>
                </select>
                <select
                  v-model="address.qu"
                  name=""
            ...
            <div class="w-2/3 mb-4 flex">
              <div class="w-2/6 pr-5 flex justify-end">
                <span><span class="text-red-600">*</span> 详细地址:</span>
              </div>
              <div class="w-4/6">
                <el-input
                  type="textarea"
            ...
            <div class="w-2/3 mb-4 flex">
              <div class="w-2/6 pr-5 flex justify-end">
                <label for="yz"
                  ><span class="text-red-600">*</span>手机号码:</label
                >
              </div>
              <div class="w-4/6">
                <el-input
            ...
            <div class="w-2/3 mb-4 flex">
              <div class="w-2/6 pr-5 flex justify-end">
                <label for="xm"
                  ><span class="text-red-600">*</span>收货人姓名:</label
                >
              </div>
              <div class="w-4/6">
                <el-input
        ...
        <button class="px-6 py-4 border w-36" @click="dialogVisible = true">
          使用新地址
        </button>
      </div>

      <p class="text-base font-semibold">确认订单信息</p>
      <div class="flex pt-12 justify-center items-center text-center">
        <div
          style="flex-grow: 4"
          class="border-b-2 border-blue-400 text-sm text-gray-500 mr-1"
        >
          <span>店铺宝贝</span>
        </div>
        <div
      ...
      <div class="pt-8 pb-2 cursor-pointer" @click="StoreDetails(seller[0].sel_id)">
        <svg
          t="1651662869339"
        ...
        <span v-if="seller[0]">{{seller[0].sel_store}}</span>
      </div>
      <div v-for="item in data" :key="item.goods_id">
        <div
          class="
            flex
            text-center
          ...
          "
        >
          <div class=' cursor-pointer' style="flex-grow: 4" @click="goshop(item.goods_id)">
            <div class="flex">
              <img class="h-28 w-28 bg-black" :src="item.goods_img" />
              <span class="pl-4 text-sm w-52">{{ item.goods_name }}</span>
            </div>
          </div>
          <div style="flex-grow: 1">
            <span>￥{{ item.goods_discount }}</span>
          </div>
          <div style="flex-grow: 1">
            <el-input-number
              size="mini"
          ...
          <div style="flex-grow: 2">
            <span class="text-lg text-red-500">
              ￥{{ (item.goods_discount * quantity).toFixed(2) }}</span
            >
          </div>
        </div>
        ...
        <p class=" text-2xl">购物车总数</p>
          <div class=" w-full   border-y-2  border-x xl:mt-20 text-sm  "  style=" background:rgb(250,250,250)">
            <div class=" flex  justify-between border-b-2 py-4 px-4">
              <p >小计</p>
              <p>￥{{(item.goods_discount*quantity).toFixed(2)}}</p>
            </div>
            <div class="  py-4 px-4">
              <p  class=" mb-3">运费</p>
            ...
            <div class="py-4 px-4 border-b-2">
              <p>运送至<span  class=" text-base"> 新疆维吾尔族自治区伊犁哈萨克自治州特克斯县。</span> </p>
                ...
                <div class="py-4 px-4 flex justify-between text-lg">
                  <p>全部的</p>
                  <p>￥{{(item.goods_discount*quantity+freight).toFixed(2)}}</p>
                  </div>
      </div>
          ...
          <div class="w-1/3">
            <div
              class="w-full border-y-2 border-x xl:mt-20 text-sm"
            ...
            >
              ...
              <div class="py-4 px-4 flex justify-between text-lg">
                <p>全部的</p>
                <p>
                  ￥{{ (item.goods_discount * quantity + freight).toFixed(2) }}
                </p>
              </div>
            </div>
            <div
              @click="go"
              class="my-4 py-4 cursor-pointer text-center text-white"
  ...
  methods: {
    ...
    submit() {
      if (
        ...
        axios({
          method: "get",
          url: "http://localhost:8080/api/addAddr",
          params: {
            addr_local:
              this.address.sheng +
              this.address.shi +
    ...
    show() {
      setTimeout(() => {
        ...
        // 3.分别获取这三个下拉框
        var sheng = this.$refs.sheng;
        var shi = this.$refs.shi;
        var qu = this.$refs.qu;
        var frag = document.createDocumentFragment();
        console.log(sheng, shi, qu, frag);
        // 4.遍历数据
    ...
    go() {
      localStorage.setItem("addr_id", this.radio);
      axios({
        method: "post",
        url: "http://localhost:8080/api/go1",
        params: {
          goods_id: this.data[0].goods_id,
          quantity: this.quantity,
          money: this.data[0].goods_discount * this.quantity + this.freight,
        },
      }).then((res) => {
    ...
    freight1() {
      this.$refs.r1[0].checked ? (this.freight = 10) : (this.freight = 20);
    },
  },
  mounted() {
    this.quantity = sessionStorage.getItem("quantity");
    this.data.push(JSON.parse(sessionStorage.getItem("data")));
    console.log(this.data,'data');
    axios({
      method: 'GET',
      url: 'http://localhost:8080/api/iDgetTab_seller',
      params: {
         sel_id:this.data[0].sel_id
      }
