<template>
  <div>
    ...
    <div class="container mx-auto px-36 py-10" >
      ...
      <div v-for="item,i in CarData" :key="item[0].goods_id">
      <div class="pt-8 pb-2 cursor-pointer" @click="StoreDetails(seller[i].sel_id)">
        <svg
          t="1651662869339"
        ...
        <span v-if="seller[0]">{{seller[i].sel_store}}</span>
      </div>
        <div
          class="
            flex
        ...
        >
          <div style="flex-grow: 4">
            <div class="flex">
              <img class="h-28 w-28 bg-black cursor-pointer" :src="item[0].goods_img"  @click="goshop(item[0].goods_id)"/>
              <span class="pl-4 text-sm w-52 cursor-pointer" @click="goshop(item[0].goods_id)">{{ item[0].goods_name }}</span>
            </div>
          </div>
          <div style="flex-grow: 1">
            <span>￥{{ item[0].goods_discount }}</span>
          </div>
          <div style="flex-grow: 1">
            <el-input-number
              size="mini"
              v-model="data[i].cart_count"
              :min="1"
              label="描述文字"
              @change="handleChange(item[0].goods_id,$event)"
            ></el-input-number>
          </div>
          ...
          <div style="flex-grow: 2">
            <span class="text-lg text-red-500">
              ￥{{ (item[0].goods_discount * data[i].cart_count).toFixed(2) }}</span
            >
          </div>
        </div>
       
      </div> <div class="w-full flex justify-end">
          <div class=" xl:w-2/4 xl:pr-20 px-6 xl:px-0">
  ...
  mounted() {
    ...
    for (var i=0; i <res.data.length; i++){
      axios({
        ...
        this.seller.push(res1.data[0])

        console.log(this.data,'data',this.seller,'seller',this.CarData,'Cardata');
