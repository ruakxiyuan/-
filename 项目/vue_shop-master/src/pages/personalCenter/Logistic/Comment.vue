<template>
  <div class='' v-if="goods.length!=0 && goods.length == tab_order.length">
      ...
      >
        ...
        >
          ...
          <div class="flex-grow" style="flex-grow: 2" >
            ...
            <span v-if="seller.length!=0"> {{seller[i].sel_store}} </span>
          </div>
        </div>
    ...
    <span class="text-gray-500 cursor-pointer" slot="reference"> 交易成功 </span>

            
          </div>
          <div class="flex-grow">
            <span 
            @click='Comment(tab_order[i].order_id)'
              class="
                w-10
                h-4
               border
